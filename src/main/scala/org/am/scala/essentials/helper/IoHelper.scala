...
object IoHelper {
  ...
  implicit def stringToRichFile(path: String) = new RichFile(new File(path))
  implicit def fileAndStringToRichFile(parent: File, path: String) = new RichFile(new File(parent, path))
